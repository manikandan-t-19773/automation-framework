...
wb = openpyxl.load_workbook('manualtestcasedoc/LogicContainers.xlsx')
ws = wb['LogicContainers']
...
out = 'manualtestcasedoc/logic_containers_testcases.json'
with open(out, 'w', encoding='utf-8') as f:
    json.dump(testcases, f, indent=2, ensure_ascii=False)

print(f"Parsed {len(testcases)} test cases -> {out}")
for tc in testcases:
    print(f"  TC{tc['id']:>3}: {tc['name']:<55} ({len(tc['steps'])} steps)")
